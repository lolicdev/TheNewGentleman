...
{
	auto rg = Rg(rgChoice);
	if (!rg || addonIdx < Common::def || (addonIdx >= 0 && rg->malAddons.find(addonIdx) == rg->malAddons.end()))
		return;
...
const std::string Core::GetRgInfo(RgKey rgChoice) const
{
	...
	if (!rg)
		return res;
	if (rg == &rgInfoList[0])
		return "Internal group for TNG to handle main race groups with same skin.";
	auto& list = rg->races;
...
bool Core::ReevaluateRace(RE::TESRace* const race, RE::Actor* const actor)
{
	...
	if (isValid) {
		...
		if (rgIt != rgInfoList.end() && rg->races.empty())
			rgInfoList.erase(rgIt);
	}
	return isValid;
}
...
Common::eRes Core::CanModifyActor(RE::Actor* const actor) const
{
	...
	if (auto skin = npc->skin; skin && skin->HasPartOf(Common::genitalSlot) && !npc->skin->HasKeyword(ut->Key(Common::kyTngSkin))) {
		for (auto& aa : skin->armorAddons)
			if (aa && aa->HasPartOf(Common::genitalSlot))
				return Common::resOkRaceR;
		return Common::errSkin;
	}
	if (npc->race->HasKeyword(ut->Key(Common::kyReady)))
		return Common::resOkRaceR;
	if (auto rg = Rg(RgKey(npc->race)); !rg || rg->malAddons.size() == 0)
		return Common::errRace;
	if (npc->race->HasKeyword(ut->Key(Common::kyProcessed)))
		return Common::resOkRaceP;
	if (npc->race->HasKeyword(ut->Key(Common::kyPreProcessed)))
		return Common::resOkRacePP;
	return Common::errRace;
}
...
{
	auto npc = actor ? actor->GetActorBase() : nullptr;
	if (!npc || actor->IsDisabled() || !npc->race || !npc->race->skin)
		return;
	auto canModify = CanModifyActor(actor);
	auto skin = npc->skin;
	if (canModify == Common::resOkRacePP)
		ReevaluateRace(actor->GetRace(), actor);
...
std::vector<std::pair<size_t, bool>> Core::GetActorAddons(RE::Actor* const actor, const bool onlyActive) const
{
	...
	if (auto rg = Rg(RgKey(npc->race)); rg) {
		...
		for (auto& addonPair : list) {
			if (!onlyActive || master[addonPair.first].second)
				res.push_back({ addonPair.first, addonPair.second.first });
		}
	}
	return res;
}
...
Common::eRes Core::GetActorAddon(RE::Actor* actor, int& addonIdx, bool& isAuto) const
{
	...
	if (IsNPCExcluded(npc) || npc->HasKeyword(ut->Key(Common::kyExcluded))) {
		isAuto = false;
		if (npc->skin && npc->skin->HasKeyword(ut->Key(Common::kyTngSkin)))
			return Common::err40;
	...
	if (!rg)
		return Common::errRace;
	if (!npc->IsFemale())
		addonIdx = rg->addonIdx;
	if (!npc->skin)
		return (addonIdx > Common::nul) ? Common::resOkHasAddon : Common::resOkNoAddon;
	if (!npc->skin->HasKeyword(ut->Key(Common::kyTngSkin)))
		return (addonIdx > Common::nul) ? Common::err40 : Common::resOkNoAddon;
	npc->ForEachKeyword([&](RE::BGSKeyword* kw) {
		if (!kw || kw->GetFormEditorID() == NULL)
...
{
	if (shouldSave) {
		if (CanModifyActor(actor) == Common::resOkRacePP)
			ReevaluateRace(actor->GetRace(), actor);
	...
	if (actor->IsPlayerRef() && choice == Common::defPlayer)
		return UpdatePlayer(actor, false);
	const auto npc = actor->GetActorBase();
	auto list = GetActorAddons(actor, !isUser);
	if (shouldSave) {
		if (choice >= static_cast<int>(list.size()))
			return Common::errAddon;
		SKSE::log::debug("Setting addon [{}] for actor [0x{:x}:{}].", choice, actor->GetFormID(), npc->GetName());
	}
	auto addonIdx = choice < 0 ? choice : shouldSave ? static_cast<int>(list[choice].first) :
																										 choice;
...
{
	sizeCat = Common::nan;
	if (auto res = CanModifyActor(actor); res < 0)
		return res;
	const auto npc = actor->GetActorBase();
	if (npc->IsPlayer() && boolSettings.Get(Common::bsExcludePlayerSize))
		return Common::errPlayer;
	if (sizeCat = Inis::GetActorSize(actor, npc); sizeCat >= 0)
		return Common::resOkSizable;
	sizeCat = ut->HasKeywordInList(npc, ut->SizeKeys());
	if (sizeCat < 0)
		sizeCat = npc->formID % Common::sizeCatCount;
	return Common::resOkSizable;
}
...
Common::eRes Core::SetActorSize(RE::Actor* const actor, int sizeCat, const bool shouldSave)
{
	...
	const auto npc = actor->GetActorBase();
	if (!actor->IsPlayerRef() && npc->IsPlayer())
		return Common::resOkFixed;	// Don't change the size for copies of player actor
	int currCat = Common::nan;
	if (sizeCat == Common::def)
		npc->RemoveKeywords(ut->SizeKeys());
	auto res = GetActorSize(actor, currCat);
	if (res != Common::resOkSizable)
		return res;
		...
		ut->DoDelayed(
				[actor, isPlayer, scale, shouldSave, sizeCat]() {
					...
					if (baseNode && scrotNode) {
						if (baseNode->local.scale == scale)
							return;
						baseNode->local.scale = scale;
						scrotNode->local.scale = 1.0f / sqrt(scale);
						if (sizeCat != Common::nul)
							SKSE::log::debug("Actor [0x{:x}] genitalia {} to [{}].", ac->GetFormID(), shouldSave ? "scaled" : "restored", scale);
					} else {
	...
	if (actor->IsPlayerRef() && sizeCat != Common::nul && shouldSave)
		SetPlayerInfo(actor, nullptr, Common::nan, sizeCat);
	if (!actor->IsPlayerRef() && shouldSave)
		Inis::SetActorSize(actor, actor->GetActorBase(), sizeCat);
	return res;
}
...
void Core::UpdatePlayerAfterLoad()
{
	...
	if (!core->boolSettings.Get(Common::bsForceRechecks) && !core->boolSettings.Get(Common::bsCheckPlayerAddon))
		return;
	auto LoadPC = []() {};
	auto LoadCond = []() {
		auto player = RE::PlayerCharacter::GetSingleton();
...
bool Core::SwapRevealing(RE::Actor* const actor, RE::TESObjectARMO* const armor)
{
	...
	if (!npc || !armor)
		return false;
	if (armor->HasKeyword(ut->Key(Common::kyUnderwear)))
		return false;
...
void Core::RevisitRevealingArmor() const
{
	...
	for (const auto& armor : armorList) {
		if (!armor || !armor->HasPartOf(Common::bodySlot) || !armor->HasKeywordInArray(rc, false))
			continue;
		auto modName = armor->GetFile(0) ? std::string(armor->GetFile(0)->GetFilename()) : "";
		if (modName.empty() || std::ranges::find(s52, modName) == s52.end())
			continue;
...
void Core::LoadAddons()
{
	...
	for (const auto& armor : armorList) {
		if (armor->HasKeyword(ut->Key(Common::kyAddonM)))
			malAddons.emplace_back(armor, true);
		if (armor->HasKeyword(ut->Key(Common::kyAddonF)))
			femAddons.emplace_back(armor, false);
		if (armor->HasKeyword(ut->Key(Common::kyPreSkin)))
			preSkins.insert(armor);
	}
	for (auto& addonPair : malAddons) {
		if (!addonPair.first->HasKeyword(ut->Key(Common::kyIgnored)))
			addonPair.first->AddKeyword(ut->Key(Common::kyIgnored));
		auto addonLoc = ut->FormToLoc(addonPair.first);
		if (activeMalAddons.find(addonLoc) != activeMalAddons.end())
			addonPair.second = false;
	}
	for (auto& addonPair : femAddons) {
		if (!addonPair.first->HasKeyword(ut->Key(Common::kyIgnored)))
			addonPair.first->AddKeyword(ut->Key(Common::kyIgnored));
		auto addonLoc = ut->FormToLoc(addonPair.first);
		if (activeFemAddons.find(addonLoc) != activeFemAddons.end())
			addonPair.second = true;
	}
...
int Core::AddonIdxByLoc(const bool isFemale, const SEFormLocView addonLoc) const
{
	if (addonLoc.second.empty())
		return Common::nan;
	if (addonLoc.second == Common::nulStr)
		return Common::nul;
	if (addonLoc.second == Common::defStr)
		return Common::def;
	auto& list = isFemale ? femAddons : malAddons;
	for (int i = 0; i < list.size(); i++) {
		if (ut->FormToLoc(list[i].first) == addonLoc)
			return i;
	}
	return Common::nan;
}
...
void Core::ProcessRaces()
{
	...
	for (const auto& race : allRaces) {
		if (!race)
			continue;
		if (!race->HasKeywordInArray(keywords, false))
			ProcessRace(race);
...
Common::eRes Core::CheckRace(RE::TESRace* const race) const
{
	try {
		for (auto raceInfo : hardCodedRaces)
			if (ut->FormToLoc(race) == raceInfo)
				return Common::resOkRaceP;
		if (race->HasKeyword(ut->Key(Common::kyCreature)) || race->IsChildRace() || !race->HasKeyword(ut->Key(Common::kyManMer)))
...
Common::RaceGroupInfo* Core::AddRace(RE::TESRace* const race, const bool isProcessed)
{
	...
	if (ut->Block() && !ut->Block()->armorAddons[0]->IsValidRace(race))
		ut->Block()->armorAddons[0]->additionalRaces.push_back(rg.armorRace);
...
{
	if (rg.defAddonIdx != Common::nan)
		return rg.defAddonIdx;
	SEFormLoc defAddon;
	...
	if (!defAddonSet)
		defAddon = std::make_pair(std::get<3>(Common::cVanillaDefaults[rgIsBeast ? 9 : 0]), Common::mainFile);
	...
	for (size_t i = 0; i < addons.size(); i++) {
		...
		for (const auto& aa : addon->armorAddons) {
			if (aa && aa->IsValidRace(rg.armorRace)) {
				...
				if (rg.isMain) {
					if (aa->additionalRaces.size() > 0)
						aaRaces.insert(aa->additionalRaces.begin(), aa->additionalRaces.end());
					for (auto& race : aaRaces) {
						if (auto r = Rg(RgKey(race)); r && !r->isMain)
							toEraseRaces.insert(r->armorRace);
					}
					for (auto& race : toEraseRaces) aaRaces.erase(race);
					if (aaRaces.find(rg.armorRace) == aaRaces.end())
						continue;
				}
				supports = true;
		...
		if (!supports) {
			if (i == rg.defAddonIdx)
				rg.defAddonIdx = rg.malAddons.empty() ? Common::nul : static_cast<int>(rg.malAddons.begin()->first);
			if (rg.addonIdx < 0 || rg.addonIdx == i)
				rg.addonIdx = rg.defAddonIdx;
			continue;
		}
		if (rg.defAddonIdx < 0)
			rg.defAddonIdx = static_cast<int>(i);
		if (i == rg.defAddonIdx && rg.addonIdx < 0)
...
RE::TESObjectARMO* Core::GetSkinWithAddonForRg(Common::RaceGroupInfo* const rg, RE::TESObjectARMO* skin, const size_t addonIdx, const bool isFemale)
{
	...
	if (!ogSkin->HasKeyword(ut->Key(Common::kyTngSkin)) && !ogSkin->HasKeyword(ut->Key(Common::kyIgnored)))
		ogSkin->AddKeyword(ut->Key(Common::kyIgnored));
	if (skinMap.find(ogSkin) == skinMap.end())
		skinMap.insert({ ogSkin, std::map<size_t, RE::TESObjectARMO*>{} });
	...
	if (skinMap[ogSkin].find(addonIdx) == skinMap[ogSkin].end()) {
		...
		if (isFemale && femAddons[addonIdx].first->HasKeyword(ut->Key(Common::kySkinWP)))
			resSkin->AddKeyword(ut->Key(Common::kySkinWP));
	...
	if (std::find(resSkin->armorAddons.begin(), resSkin->armorAddons.end(), reqAA) == resSkin->armorAddons.end())
		resSkin->armorAddons.push_back(reqAA);
	return resSkin;
}
...
RE::TESObjectARMO* Core::GetOgSkin(RE::TESObjectARMO* skin) const
{
	...
	if (!res) {
		...
		if (res->HasKeyword(ut->Key(Common::kyTngSkin)))
			problem = 2;
	}
	if (problem) {
...
void Core::ProcessNPCs()
{
	...
	for (const auto& npc : allNPCs) {
		...
		if (!race->HasKeyword(ut->Key(Common::kyProcessed)) && !race->HasKeyword(ut->Key(Common::kyPreProcessed)))
			continue;
		...
		if (!skin)
			continue;
		if (skin->HasKeyword(ut->Key(Common::kyTngSkin)))
			continue;
		skinsToPatch[{ skin, race }].insert(npc);
	}
	for (auto& racePair : raceNPCCount) {
		if (!racePair.first || racePair.second < 5)
			continue;
		if (auto rg = Rg(RgKey(racePair.first)); rg && rg->malAddons.size() > 0)
			rg->noMCM = false;
	}
	...
	if (skinsToPatch.size() > 0) {
		...
		if (tot > 0)
			SKSE::log::debug("\t\t[{}] skins were not patched.", tot);
	}
	...
	for (size_t i = 1; i < rgInfoList.size(); i++)
		if (rgInfoList[i].noMCM)
			SKSE::log::info("\tRace group [{}] with the following information: {}.", rgInfoList[i].name, GetRgInfo(RgKey(i, false)));
}
...
RE::TESObjectARMO* Core::FixSkin(RE::TESObjectARMO* const skin, RE::TESRace* const race, const char* const name)
{
	...
	if (auto rg = Rg(RgKey(race)); !rg) {
		...
		if (name)
			SKSE::log::info("\t\tThe skin [0x{:x}: {}] added as extra skin.", skin->GetFormID(), name);
...
std::pair<int, bool> Core::GetApplicableAddon(RE::Actor* const actor) const
{
	...
	if (count == 0 || chance == 0)
		return { addonIdx, false };
	if (npc->GetFormID() % 100 < chance)
		addonIdx = static_cast<int>(list[npc->GetFormID() % count].first);
	return { addonIdx, false };
}

Common::eRes Core::SetNPCAddon(RE::TESNPC* const npc, const int addonIdx, const bool isUser)
{
	if (addonIdx < Common::def)
		return Common::errAddon;
	...
	if (addonIdx == Common::def && !npc->skin)
		return !npc->IsFemale() ? res : Common::resOkFixed;
	...
	if (!rg)
		return Common::errRg;
	auto& list = npc->IsFemale() ? rg->femAddons : rg->malAddons;
	if (addonIdx >= 0 && list.find(addonIdx) == list.end())
		return Common::errAddon;
	if (addonIdx == Common::def && npc->IsFemale()) {
		OrganizeNPCKeywords(npc, addonIdx, false);
		if (npcSkin && npcSkin->HasKeyword(ut->Key(Common::kyTngSkin)))
			npc->skin = activeOgSkin == raceSkin ? nullptr : activeOgSkin;
...
void Core::OrganizeNPCKeywords(RE::TESNPC* const npc, int addonIdx, const bool isUser) const
{
	...
	if (addonIdx == Common::nul) {
		...
		if (!kw)
			SKSE::log::critical("Keyword generation routine failed with keyword {}", reqKw);
		npc->AddKeyword(kw);
	}
	if (npc->IsFemale()) {
...
void Core::UpdateAddon(RE::Actor* const actor, const bool isRRace)
{
	...
	if (addonRes < 0 && addonRes != Common::err40)
		return;
	if (!isAuto) {
		if (addonRes == Common::err40)
			SetActorAddon(actor, currIdx, true, false);
		return;
	}
	auto [autoIdx, isSaved] = GetApplicableAddon(actor);
	if (autoIdx == Common::def) {
		if (addonRes == Common::err40)
			SetActorAddon(actor, currIdx, false, false);
		return;
	}
	if (addonRes == Common::err40 || currIdx != autoIdx)
		SetActorAddon(actor, autoIdx, isSaved, false);
}
...
void Core::UpdateBlock(RE::Actor* const actor, RE::TESObjectARMO* const armor, const bool isEquipped) const
{
	...
	if ((hasCover && down) || (!hasCover && !down))
		return;
...
void Core::CheckArmorPieces()
{
	...
	for (const auto& armor : armorList) {
		...
		if (!has52) {
			...
			if (armor->HasPartOf(Common::bodySlot))
				continue;
			if (modName != "")
				potentialSlot52Mods.insert(modName);
			SKSE::log::info("\t\tThe armor [0x{:x}] would cover genitals!", armor->GetFormID());
			continue;
		}
		if (armor->HasPartOf(Common::bodySlot)) {
			if (modName != "")
				potentialMods.insert(std::string{ modName });
	...
	if (logInfo[0] > 0)
		SKSE::log::warn("\t\t[{}]: were problematic!", logInfo[0]);
	if (logInfo[1] > 0)
		SKSE::log::info("\t\t[{}]: were already marked with TNG keywords.", logInfo[1]);
	if (logInfo[2] > 0)
		SKSE::log::info("\t\t[{}]: are covering due to ini-files, having slot 52, or previous user input.", logInfo[2]);
	if (logInfo[3] > 0)
		SKSE::log::info("\t\t[{}]: are revealing, due to ini-files, having SOS_Revealing, or previous user input.", logInfo[3]);
	if (logInfo[4] > 0)
		SKSE::log::info("\t\t[{}]: were recognized to be covering.", logInfo[4]);
	if (logInfo[5] > 0)
		SKSE::log::info("\t\t[{}]: were recognized to be revealing.", logInfo[5]);
	if (logInfo[6] > 0)
		SKSE::log::info("\t\tThe rest [{}] are not relevant and are ignored!", logInfo[6]);
