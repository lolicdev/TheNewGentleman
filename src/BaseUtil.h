...
namespace Common
{
	...
	public:
		TypedSetting(std::array<T, n> def, std::array<std::string, n> s, std::array<std::string, n> k) :
			defValues(def), sections(s), keys(k) {}
		...
		{
			if (values[static_cast<size_t>(idx)] != value)
				values[static_cast<size_t>(idx)] = value;
		}
	};
