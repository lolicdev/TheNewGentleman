...
void Inis::LoadMainIni()
{
	...
	for (auto& key : keys) {
		...
		if (!modName.empty())
			slot52Mods.emplace(modName, status);
	}
...
void Inis::SaveMainIni()
{
	...
	for (auto& pair : slot52Mods) {
		if (!pair.first.empty())
			settingIni.SetBoolValue(cRevealingModSection, ut->NameToStr(pair.first).c_str(), pair.second);
	}
	settingIni.SaveFile(SettingFile());
}
...
void Inis::SetLogLvl(const int newLevel)
{
	...
	ini.SetUnicode();
	ini.LoadFile(SettingFile());
	if (newLevel < 1 || newLevel >= static_cast<int>(spdlog::level::n_levels))
		return;
...
void Inis::TransferOldIni()
{
	...
	if (lIniVersion < 3) {
		...
		if (ini.SectionExists(cNPCAddonSection))
			ini.Delete(cNPCAddonSection, nullptr);
	}
	if (lIniVersion < 4) {
		if (std::filesystem::exists(R"(.\Data\SKSE\Plugins\Defaults_TNG.ini)"))
			SKSE::log::warn("The [Defaults_TNG.ini] file is not used anymore by TNG, feel free to delete it.");
	}
	if (lIniVersion < 5) {
...
{
	if (!npc || !actor || actor->IsPlayerRef() || npc->IsPlayer())
		return { 0, "" };
	auto npcLoc = ut->FormToLoc(npc);
	if (!npcLoc.second.empty() && npcAddons.find(npcLoc) != npcAddons.end())
		return npcAddons.at(npcLoc);
	auto actorLoc = ut->FormToLoc(actor);
	if (!actorLoc.second.empty() && actorAddons.find(actorLoc) != actorAddons.end())
		return actorAddons.at(actorLoc);
	return { 0, "" };
}
...
int Inis::GetActorSize(const RE::Actor* actor, const RE::TESNPC* npc) const
{
	if (!npc || !actor || actor->IsPlayerRef() || npc->IsPlayer())
		return Common::nul;
	auto npcLoc = ut->FormToLoc(npc);
	if (!npcLoc.second.empty() && npcSizeCats.find(npcLoc) != npcSizeCats.end())
		return npcSizeCats.at(npcLoc);
	auto actorLoc = ut->FormToLoc(actor);
	if (!actorLoc.second.empty() && actorSizeCats.find(actorLoc) != actorSizeCats.end())
		return actorSizeCats.at(actorLoc);
	return Common::nul;
}

void Inis::SetActorSize(const RE::Actor* actor, const RE::TESNPC* npc, const int genSize)
{
	if (genSize == Common::nul)
		return;
	auto charLoc = ut->FormToLoc(npc);
	bool saveAsActor = false;
	if (charLoc.second.empty()) {
...
void Inis::LoadPlayerInfos(const std::string& saveName)
{
	...
	if (const auto save = ut->Split(saveName, "_"); save.size() == 9)
		playerStr = save[1];
	CSimpleIniA settingIni;
	settingIni.SetUnicode();
	settingIni.LoadFile(SettingFile());
...
void Inis::SetPlayerInfo(const RE::Actor* actor, const RE::TESObjectARMO* addon, const int addonChoice, const int sizeChoice)
{
	...
	if (!pcInfo)
		return;
	if (addonChoice == Common::nan && sizeChoice == Common::nan)
		return;
...
void Inis::LoadSingleIni(const char* path, const std::string_view fileName)
{
	...
	if (ini.SectionExists(cExcludeSection)) {
		if (ini.GetAllValues(cExcludeSection, cExcModRaces, values))
			LoadModNames(values, excludedRaceMods, cExcModRaces, fileName);
		if (ini.GetAllValues(cExcludeSection, cExcRace, values))
			LoadModRecordPairs(values, excludedRaces, cExcRace, fileName);
		if (ini.GetAllValues(cExcludeSection, cExcludeNPC, values))
			LoadModRecordPairs(values, excludedNPCs, cExcludeNPC, fileName);
	}
	if (ini.SectionExists(cSkinSection)) {
		if (ini.GetAllValues(cSkinSection, cSkinMod, values))
			LoadModNames(values, skinMods, cSkinMod, fileName);
		if (ini.GetAllValues(cSkinSection, cSkinRecord, values))
			LoadModRecordPairs(values, skinRecords, cSkinRecord, fileName);
	}
	if (ini.SectionExists(cArmorSection)) {
		if (ini.GetAllValues(cArmorSection, cRevealingMod, values))
			LoadModNames(values, revealingMods, cRevealingMod, fileName);
		if (ini.GetAllValues(cArmorSection, cFemRevMod, values))
			LoadModNames(values, femRevMods, cFemRevMod, fileName);
		if (ini.GetAllValues(cArmorSection, cMalRevMod, values))
			LoadModNames(values, malRevMods, cMalRevMod, fileName);
		if (ini.GetAllValues(cArmorSection, cCoveringRecord, values))
			LoadModRecordPairs(values, coveringRecords, cCoveringRecord, fileName);
		if (ini.GetAllValues(cArmorSection, cRevealingRecord, values))
			LoadModRecordPairs(values, revealingRecords, cRevealingRecord, fileName);
		if (ini.GetAllValues(cArmorSection, cFemRevRecord, values))
			LoadModRecordPairs(values, femRevRecords, cFemRevRecord, fileName);
		if (ini.GetAllValues(cArmorSection, cMalRevRecord, values))
			LoadModRecordPairs(values, malRevRecords, cMalRevRecord, fileName);
	}
...
bool Inis::IsRaceExcluded(const RE::TESRace* race) const
{
	...
	if (excludedRaceMods.find(modName) != excludedRaceMods.end())
		return true;
	if (excludedRaces.find(ut->FormToLoc(race)) != excludedRaces.end())
		return true;
	return false;
}
...
{
	if (!npc || !npc->GetFile(0))
		return false;
	if (excludedNPCs.find(ut->FormToLoc(npc)) != excludedNPCs.end())
		return true;
	return false;
}
...
bool Inis::IsSkin(const RE::TESObjectARMO* armor, const std::string& modName)
{
	if (modName == "")
		return false;
	if (skinMods.find(modName) != skinMods.end())
		return true;
	if (skinRecords.find({ armor->GetLocalFormID(), modName }) != skinRecords.end())
		return true;
	return false;
}
...
Common::eKeyword Inis::HasStatus(const RE::TESObjectARMO* armor) const
{
	...
	if (armorLoc.second.empty())
		return Common::keywordsCount;
	if (runTimeArmorStatus.find(armorLoc) != runTimeArmorStatus.end())
		return statusKeys[static_cast<size_t>(runTimeArmorStatus.at(armorLoc))];
	if (coveringRecords.find(armorLoc) != coveringRecords.end())
		return Common::kyCovering;
	if (!armor->HasPartOf(Common::bodySlot))
		return Common::keywordsCount;
	if (revealingRecords.find(armorLoc) != revealingRecords.end())
		return Common::kyRevealing;
	if (femRevRecords.find(armorLoc) != femRevRecords.end())
		return Common::kyRevealingF;
	if (malRevRecords.find(armorLoc) != malRevRecords.end())
		return Common::kyRevealingM;
	if (revealingMods.find(armorLoc.second) != revealingMods.end())
		return Common::kyRevealing;
	if (femRevMods.find(armorLoc.second) != femRevMods.end())
		return Common::kyRevealingF;
	if (malRevMods.find(armorLoc.second) != malRevMods.end())
		return Common::kyRevealingM;
	if (armor->HasKeywordString(Common::sosRevealing))
		return Common::kyRevealing;
	return Common::keywordsCount;
}
