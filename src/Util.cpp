...
RE::TESRace* Common::Util::Race(const size_t idx)
{
	if (idx >= racesCount)
		return nullptr;
	if (!races[idx])
		races[idx] = ut->SEDH()->LookupForm<RE::TESRace>(raceIDs[idx].first, raceIDs[idx].second);
	return races[idx];
}

RE::BGSKeyword* Common::Util::Key(const size_t idx)
{
	if (idx >= keywordsCount)
		return nullptr;
	if (!keywords[idx])
		keywords[idx] = ut->SEDH()->LookupForm<RE::BGSKeyword>(keyIDs[idx].first, keyIDs[idx].second);
	return keywords[idx];
}
...
{
	if (!sizeKey[idx])
		sizeKey[idx] = ut->SEDH()->LookupForm<RE::BGSKeyword>(sizeKeyIDs[idx], mainFile);
	return sizeKey[idx];
}
...
{
	if (!block)
		block = ut->SEDH()->LookupForm<RE::TESObjectARMO>(coverID.first, coverID.second);
	return block;
}
...
bool Common::Util::IsCovering(const RE::Actor* const actor, const RE::TESObjectARMO* const armor)
{
	...
	if (!npc || !armor)
		return false;
	if (armor->HasKeyword(Key(Common::kyCovering)))
		return true;
	if (armor->HasKeyword(Key(Common::kyRevealingF)) && !npc->IsFemale())
		return true;
	if (armor->HasKeyword(Key(Common::kyRevealingM)) && npc->IsFemale())
		return true;
	return false;
}
...
bool Common::Util::HasCovering(RE::Actor* const actor, RE::TESObjectARMO* const exception)
{
	...
	for (const auto& armor : wornArmor)
		if (armor && IsCovering(actor, armor) && armor != exception)
			return true;
	return false;
}
