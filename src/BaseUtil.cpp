...
int Common::BaseUtil::HasKeywordInList(const RE::BGSKeywordForm* form, const std::vector<RE::BGSKeyword*>& keywords) const
{
	for (int i = 0; i < keywords.size(); i++) {
		if (keywords[i] && form->HasKeyword(keywords[i]))
			return i;
	}
	return -1;
}
...
{
	if (loc.first == 0 && loc.second == nulStr)
		return nulStr;
	if (loc.first == 0 && loc.second == defStr)
		return defStr;
	if (loc.first == 0 || loc.second.empty())
		return "";
...
std::string Common::BaseUtil::NameToStr(std::string name) const
{
	...
	if (name.length() > 2 && name.front() == '\"' && name.back() == '\"')
		return name;
	if (name.find_first_of(iniChars) != std::string::npos)
		return "\"" + name + "\"";
	return name;
}
...
std::string Common::BaseUtil::StrToName(std::string name) const
{
	...
	if (name.front() == '\"' && name.back() == '\"')
		res = name.substr(1, name.length() - 2);
	return res;
}
