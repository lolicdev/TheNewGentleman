...
bool Papyrus::GetBoolValue(RE::StaticFunctionTag*, int settingID)
{
	if (0 <= settingID && settingID < Common::boolSettingCount)
		return core->boolSettings.Get(static_cast<Common::eBoolSetting>(settingID));
	return false;
}
...
void Papyrus::SetBoolValue(RE::StaticFunctionTag*, int settingID, bool value)
{
	if (0 <= settingID && settingID < Common::boolSettingCount) {
		...
		case Common::bsRevealSlot52Mods:
			if (core->boolSettings.Get(Common::eBoolSetting::bsRevealSlot52Mods) != value)
...
int Papyrus::GetIntValue(RE::StaticFunctionTag*, int settingID)
{
	if (0 <= settingID && settingID < Common::intSettingCount)
		return core->intSettings.Get(static_cast<Common::eIntSetting>(settingID));
	return Common::nan;
}
...
float Papyrus::GetFloatValue(RE::StaticFunctionTag*, int settingID)
{
	if (0 <= settingID && settingID < Common::floatSettingCount)
		return core->floatSettings.Get(static_cast<Common::eFloatSetting>(settingID));
	return 1.0f;
}
...
{
	auto& list = core->GenderAddons(isFemale);
	if (addonIdx < 0 || addonIdx >= list.size())
		return;
...
std::vector<std::string> Papyrus::GetRgAddons(RE::StaticFunctionTag*, int rgIdx)
{
	...
	for (auto i : list) {
		...
		name = isMain ? name : i.second ? name + " (d)" :
																			name + " (s)";
		res.push_back(name);
	}
	return res;
...
{
	auto wornArmor = ut->GetWornAmor(actor);
	if (choice < 0 || choice > wornArmor.size())
		return false;
	...
	case iidCanSee:
		{
			if (!hasCover)
				return "$TNG_PA1";
			if (down)
				return ut->IsBlock(down) ? "$TNG_PD0" : "$TNG_PA2";
	...
	case iidCannotSee:
		{
			...
			if (!down) {
				...
				if (skin->HasKeyword(ut->Key(Common::kyTngSkin)))
					return "$TNG_PD0";
				if (npc->HasKeyword(ut->Key(Common::kyExcluded)))
					return "$TNG_PA3";
				if (npc->IsFemale() && static_cast<size_t>(std::floor(core->floatSettings.Get(Common::fsFemRndChance) + 0.1f)) < 100)
					return "$TNG_PA4";
				if (core->GetRgAddon(Core::RgKey(npc->race)) == Common::nul)
					return "$TNG_PA5";
			}
			core->UpdateActor(actor);
